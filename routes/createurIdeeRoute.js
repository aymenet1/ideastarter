...
module.exports = (function() {
        ...
        function ajouter_idee(cat) {
            idee.save().then(function(newIdee) {
                ...
                fs.readFile(req.files.image_idee[0].path, function(err, data) {
                    ...
                    image.url = newPath;
                    image.type = path.extname(req.files.image_idee[0].path)
    ...
    createurIdeeRoute.get("/mesidees", function(req, res) {
        models.Idee.findAll({
            where: {
                idUtilisateur: req.session.Utilisateur.id
            },
    ...
    createurIdeeRoute.get("/contacter", function(req, res) {
        res.render('createuridee/contacter');
    });
            ...
            models.Rating.findOne({
                ...
                if (findR == null) {
                    rating.save().then(function(r) {
                        r.setIdee(idee).then(function(data) {
                            models.Utilisateur.findById(req.session.Utilisateur.id).then(function(u) {
                                r.setUtilisateur(u).then(function() {
                                    //res.send(200).json({ status: "succes" })
                                })
                            });
                        })
                    })
                    ...
                    models.Rating.update({ note: rating.note }, { where: { id: findR.id } }).then(function() {
                        res.send(200).json({ status: "succes" })
                    });
